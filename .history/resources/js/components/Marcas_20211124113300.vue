    ...
    <div class="container">
        ...
        <modal-component id="modalMarcaAdc" titulo="Adicionar | marca">
            ...
            <template v-slot:alertas>
                <alert-component
                    tipo="success"
                    :detalhes="$store.transacao"
                    titulo="Cadastro realizado com sucesso"
                    v-if="$store.transacao.status == 'sucesso'">
                </alert-component>
                <alert-component
                    tipo="danger"
                    :detalhes="$store.transacao"
                    titulo="Erro ao tentar cadastrar a marca"
                    v-if="$store.transacao.status == 'erro'">
                </alert-component>
            </template>
            ...
            <template v-slot:rodape>
                <button type="button" class="btn btn-secondary" data-dismiss="modal" @click="paginacao(ultimaPagina)">Fechar</button>
        ...
        methods: {
            remover() {
                ...
                if (confirma) {
                    ...
                    axios.post(url, formData, config)
                        .then(response => {
                            this.$store.transacao.status = 'sucesso'
                            this.$store.transacao.mensagem = response.data.msg
                            this.urlPaginacao = 'page=1'
                        })
                        .catch(errors => {
                            this.$store.transacao.status = 'erro'
                            this.$store.transacao.mensagem = errors.response.data.erro
                            this.$store.transacao.dados = errors.response.data.errors
                        })
            ...
            carregarLista() {
                ...
                axios.get(url, config)
                    .then(response => {
                        ...
                        this.$store.transacao.status = 'sucesso'
                        this.$store.transacao.mensagem = 'Lista carregada com sucesso'
                        this.ultimaPagina = this.urlBase + '?' +'page='+this.marcas.last_page + this.urlFiltro
                    })
                    .catch(errors => {
                        this.$store.transacao.status = 'erro'
                        this.$store.transacao.mensagem = errors.response.data.message
                        this.$store.transacao.dados = errors.response.data.errors
                    })
            ...
            salvar() {
                ...
                axios.post(this.urlBase, formData, config)
                    .then(response => {
                        this.$store.transacao.status = 'sucesso'
                        this.$store.transacao.mensagem = 'ID do registro: ' + response.data.id
                    })
                    .catch(errors => {
                        this.$store.transacao.status = 'erro'
                        this.$store.transacao.mensagem = errors.response.data.message
