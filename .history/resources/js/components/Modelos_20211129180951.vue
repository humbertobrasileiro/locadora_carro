    ...
    export default {
        ...
        methods: {
            lerMarca(e) {
                ...
                let url = 'http://localhost:8000/api/v1/marca/' + e

                axios.get(url)
                    .then(response => {
                        this.marca = response.data
                        this.$store.state.transacao.status = 'leitura'
        ...
        mounted() {
            ...
            if (this.$store.state.item.marca_id > 0) {

                this.lerMarca(this.$store.state.item.marca_id)
